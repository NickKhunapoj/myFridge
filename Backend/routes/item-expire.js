...
const fetchExpireCount = async (token, res) => {
  try {
    ...
    jwt.verify(tokenContent, process.env.JWT_SIGN_SECRET, async (err, data) => {
      ...
      const expireData = await database.executeQuery({
        query: `
                    SELECT SUM(
                        CASE 
                            WHEN s.expire_3days = 1 AND i.expiry_date BETWEEN DATE('now') AND DATE('now', '+3 days') THEN 1
                            WHEN s.expire_5days = 1 AND i.expiry_date BETWEEN DATE('now') AND DATE('now', '+5 days') THEN 1
                            WHEN s.expire_1week = 1 AND i.expiry_date BETWEEN DATE('now') AND DATE('now', '+7 days') THEN 1
                            ELSE 0 
                        END
                    ) AS expireCount
                    FROM items_info i
                    INNER JOIN app_settings_info s ON i.user_id = s.user_id
                    WHERE i.user_id = ?
                `,
        values: [data.user_id],
      });
      ...
      if ("error" in expireData) {
        return res.status(500).send({
          ok: false,
          error: expireData.error.userError,
        });
      }
    ...
    return res.status(500).send({
      ok: false,
      error: "An error occurred while fetching expire count.",
    });
  }
};
